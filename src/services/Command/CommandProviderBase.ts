...
export abstract class CommandProviderBase {
  ...
  protected abstract RegisterCommandBase(
    commandOptions: CommandOptions
  ): vscode.Disposable;

  protected async executeCommand(
    command: IVscodeCommand,
    commandOptions: CommandOptions
  ): Promise<Thenable<boolean>> {
    if (commandOptions.UseAuthentication) {
      var session = await this.AuthService.GetAuthenticatedSession();
      if (!session) {
      ...
      if (
        !this.AuthService.TryAuthorizeCommand(
          [...session.scopes],
          commandOptions.UseAuthentication
        )
      ) {
        vscode.window.showErrorMessage(
          "Your current plan does not support this command. Upgrade at https://codesenseai.com"
        );
        ...
        if (!this.context.globalState.get("firstextension.authredirect")) {
          ...
          vscode.env.openExternal(
            vscode.Uri.parse("https://codesenseai.com/#pricing")
          );
        }
        return false;
      }
